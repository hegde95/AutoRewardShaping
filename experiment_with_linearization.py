...
def visualize_trajs(H,zm, compliant_set = None):
    num_tr = len(H)
    fig, axs = plt.subplots(2,num_tr, figsize=(8*num_tr, 8*num_tr))
...
def function_min(cost_est1, cost_est2):
    deffinite_minimum = np.minimum(cost_est1, cost_est2)

    log_sum_exp_minimum = -np.log(np.exp(-cost_est1) + np.exp(-cost_est2))

    # diff_log_sum_exp_minimum = [np.exp(-cost_est1)/(np.exp(-cost_est1) + np.exp(-cost_est2)) , np.exp(-cost_est2)/(np.exp(-cost_est1) + np.exp(-cost_est2))]
    return log_sum_exp_minimum
...
# for N in range(14, 54):
    ...
    if True:
        ...
        for hole_est1, hole_est2 in power_set:
            ...
            # if (np.argwhere(hole_est1 == np.amax(hole_est1)) == 35) and (np.argwhere(hole_est2 == np.amax(hole_est2)) == 3) and N == 6:
            #     print("Here")
            # if (np.argwhere(hole_est1 == np.amax(hole_est1)) == 35) and (np.argwhere(hole_est2 == np.amax(hole_est2)) == 18) and N == 6:
            #     print("Here")
            if all(C<=cost_est1) and all(C<=cost_est2):
                # sum_obj = sum(cost_est1) + sum(cost_est2)
                sum_obj = sum(function_min(cost_est1, cost_est2)) # works!
                # sum(function_min(np.dot(z_mat, ex_hole_est1.reshape((64))), np.dot(z_mat, ex_hole_est2.reshape((64)))))
        ...
        for x_i in hole_states_tup:
            ...
            hole_states_pairs.append((h_1[0], h_2[0]))
